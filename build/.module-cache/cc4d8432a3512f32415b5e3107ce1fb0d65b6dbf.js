...
var TimeUtil = {
	...
	init: function init(type, time) {
		...
		switch (type) {
			...
			case 'hours':
				return this.getNum(23, this.hours);
			case 'minutes':
	...
	getYear: function getYear() {
		...
		var year = this.year,
		    index = 0;
		for (var i = 2015, arr = [], j = 0; i <= (nowYear + 3); i++, j++) {
			i == year && (index = j);
			arr.push(i);
		}
	...
	_createClass(DatePickerItem, [{
		...
		value: function render() {
			...
			return React.createElement(
				'div',
				{ className: 'picker-viewport datepicker-col-1' },
				React.createElement(
					'div',
				...
				React.createElement(
					'ul',
					{ className: 'picker-scroll', style: this.state.style, ref: 'ul',
...
var DatePicker = React.createClass({
	...
	componentWillReceiveProps: function componentWillReceiveProps(nextprops) {
		var date = nextprops.nextDate && nextprops.type=='date'? new Date(nextprops.nextDate) : new Date();
		this.initData(date);
		this.setState({
	...
	render: function render() {
		...
		if (this.props.type == 'date') {
			arr.push(React.createElement(
				...
				React.createElement(DatePickerItem, { data: value, type: 'year', onSelect: this.onSelect }),
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					'div',
					{ className: 'picker-navbar' },
					React.createElement(
						'a',
						{ className: 'picker-navbar-btn mui-pull-right', onClick: this.onComfireClick },
						'完成'
					),
					React.createElement(
						'a',
						{ className: 'picker-navbar-btn mui-pull-left', onClick: this.controlShow },
						'取消'
					)
				),
				arr
...
var DateTime = React.createClass({
	...
	dateShow: function dateShow() {
		...
		this.setState({
			date: true,
			time: false,
			nextDate: this.refs.date.textContent
		});
	},
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				'p',
				{ onClick: this.timeShow },
				'显示时间:',
				React.createElement(
					'span',
					{ ref: 'time' },
					this.state.timeSection
				)
			),
			React.createElement(DatePicker, { type: 'date', isShow: this.state.date, onSelect: this.onSelect, nextDate: this.state.nextDate }),
			React.createElement(DatePicker, { type: 'time', isShow: this.state.time, onSelect: this.onSelect, nextDate: this.state.nextDate })
		);
	}
});
ReactDOM.render(React.createElement(DateTime, null), document.getElementById('main'));